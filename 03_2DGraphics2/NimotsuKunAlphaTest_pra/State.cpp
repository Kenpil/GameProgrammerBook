...
/*void State::drawCell(int x, int y, ImageID id) const {
	ImageID idTmp = id;
	if (id == State::IMAGE_ID_WALL || id == State::IMAGE_ID_GOAL || id == State::IMAGE_ID_BLOCK) {//idが壁かゴールかブロックならば描く
		mImage->draw(x * 32, y * 32, id * 32, 0, 32, 32);
	}else{//idが壁かゴールかブロック以外ならスペースを書く
		idTmp = State::IMAGE_ID_SPACE;
		mImage->draw(x * 32, y * 32, idTmp * 32, 0, 32, 32);
	}
	mImage->draw(x * 32, y * 32, id * 32, 0, 32, 32);
}*/

void State::drawCell(int x, int y, ImageID id) const {
	ImageID idTmp = State::IMAGE_ID_SPACE;
